...
router.get('/', function (req, res) {       // Sending Page Query Parameter is mandatory http://localhost:3636/api/products?page=1
    ...
    database.table('products as p')
        ...
        .slice(startValue, endValue) //range
        .sort({id: .1})
        .getAll()    //rows
        .then(prods => {  //gives promise
            if (prods.length > 0) {
                res.status(200).json({
                    count: prods.length,
                    products: prods
...
router.get('/:prodId', (req, res) => {
    ...
    database.table('products as p')
        ...
        .withFields(['c.title as category',
            ...
            'p.image',
            'p.id',
            'p.images' //IN home page only display one image 
        ])
        .filter({'p.id': productId}) //key and value
        .get()
        .then(prod => {
            console.log(prod);
            if (prod) {
...
router.get('/category/:catName', (req, res) => { // Sending Page Query Parameter is mandatory http://localhost:3636/api/products/category/categoryName?page=1
    ...
    // Get category title value from param url
    const cat_title = req.params.catName;  //params object
